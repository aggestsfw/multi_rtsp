...
class APP_EISeg(QMainWindow, Ui_EISeg):
    ...
    def __init__(self, parent=None):
        ...
        self.sldNMS.valueChanged.connect(self.nmsChanged)#阈值改变后重新运行检测
    ...
    def turnImg(self, delta, list_click=False):
        if (self.grid is None or self.grid.curr_idx is None) or list_click:
            ...
            # print("turnImg1 %d\n" % self.isDirty)
            #增加上一次推理结果是否有检测目标的判断，是的话就设置为保存
            if len(self.last_box_list) != 0:
                self.isDirty = True
            # 1. 检查是否有图可翻，保存标签
            self.currIdx += delta
    ...
    def createRects(self, save_hand=False):
        ...
        # 保存推理结果，用于后面的判断是否需要保存标签
        self.last_box_list = box_list.copy()
        # 清除全部
        self.delAllPolygon()
        # 添加
    ...
    def completeLastMask(self):
        ...
        res = self.warn(
            ...
            QMessageBox.Yes | QMessageBox.Cancel, )
        if res == QMessageBox.Yes:
            print("completeLastMask的dirty1\n")
            self.finishObject()
            self.exportLabel()
            print("completeLastMask的dirty2\n")
            self.setDirty(False)
            return True
        return False
    ...
    def reRunDet(self, save_hand=False):
        if self.image is not None and self.isUsePreAnnotation:
            # print("")
            #重新推理会再调一次createRects将isDirty设置为True
    ...
    def createRect(self, p, size, label=None, hand_rect=False):
        ...
        # print("create rect dirty2 %d\n" % self.isDirty)
        self.setDirty(True)  # 开启保存，这个注释掉不影响，因为前面已经变True，但是不能设置为False

    def canvasDraw(self):
        if self.scene.is_draw and self.controller.labelList:
            # print("当绘制矩形时候，这一条会出现")
